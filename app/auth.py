    ...
    try:
        ...
        if request.method == 'POST': 
            ...
            if attempt is not None:
                ...
                db.execute(
                    QueryInfoKey, (attempt['username'], attempt['password'], attempt['salt'], attempt['email'])
                )
                db.commit()
...
def register():
    try:
        ...
        return render_template(tempfile) 
    except:
        return render_template('auth/register.html')
    ...
    try:
        ...
        if request.method == 'POST': 
            ...
            if not password1:
                ...
                return render_template(tempfilepager, number=authid)

            if password1 != password:
                flash('Both values should be the same')
                return render_template(Template, number=authid)
